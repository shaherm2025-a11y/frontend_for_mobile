// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Arabic (`ar`).
class AppLocalizationsAr extends AppLocalizations {
  AppLocalizationsAr([String locale = 'ar']) : super(locale);

  @override
  String get welcomeText => 'تطبيق العين الزراعية';

  @override
  String get diagnosePlant => 'تشخيص النبات';

  @override
  String get contactExperts => 'التواصل مع الخبراء';

  @override
  String get ourProducts => 'منتجاتنا';

  @override
  String get awarenessGuide => 'دليل التوعية';

  @override
  String get changeLanguage => 'تغيير اللغة';

  @override
  String get plantDiagnosis => 'تشخيص النبات';

  @override
  String get selectImage => 'اختر صورة';

  @override
  String get result => 'النتيجة';

  @override
  String get treatment => 'العلاج';

  @override
  String get experts => 'الخبراء';

  @override
  String get expertsPlaceholder => 'دعم الخبراء سيتوفر قريبًا.';

  @override
  String get productsPlaceholder => 'قائمة المنتجات ستتوفر قريبًا.';

  @override
  String get awarenessPlaceholder => 'سيتم إضافة محتوى توعوي لاحقًا.';

  @override
  String get basicFarming => 'أساسيات الزراعة';

  @override
  String get soilAdvice => '• اختيار التربة المناسبة: تحليلها كل عامين.';

  @override
  String get sunAdvice => '• تعرض النبات للشمس: 6-8 ساعات يوميًا.';

  @override
  String get wateringAdvice => '• الري المنتظم: عند جفاف التربة 2-3 سم.';

  @override
  String get diseasePrevention => 'الوقاية من أمراض النبات';

  @override
  String get toolSanitation => '• تعقيم الأدوات قبل الزراعة.';

  @override
  String get cropRotation => '• ممارسة الدورة الزراعية.';

  @override
  String get seedSelection => '• استخدام بذور سليمة وموثوقة.';

  @override
  String get naturalPestControl => 'مكافحة الآفات طبيعيًا';

  @override
  String get plantRepellents => '• زراعة نباتات طاردة (نعناع، ريحان).';

  @override
  String get organicSprays => '• استخدام بخاخات عضوية مثل زيت النيم.';

  @override
  String get beneficialInsects => '• تشجيع وجود الحشرات النافعة.';

  @override
  String get commonDiseases => 'الأمراض الشائعة وطرق العلاج';

  @override
  String get disease => 'المرض';

  @override
  String get symptoms => 'الأعراض';

  @override
  String get seasonalTips => 'نصائح موسمية';

  @override
  String get spring => 'الربيع';

  @override
  String get summer => 'الصيف';

  @override
  String get autumn => 'الخريف';

  @override
  String get winter => 'الشتاء';

  @override
  String get spring1 => 'تقليم النباتات المزهرة.';

  @override
  String get spring2 => 'إضافة السماد العضوي.';

  @override
  String get summer1 => 'الري في الصباح الباكر.';

  @override
  String get summer2 => 'استخدام المهاد للحفاظ على الرطوبة.';

  @override
  String get autumn1 => 'زراعة المحاصيل الشتوية.';

  @override
  String get autumn2 => 'تنظيف الحديقة من بقايا المحاصيل.';

  @override
  String get winter1 => 'حماية النباتات الحساسة بالصوبات.';

  @override
  String get winter2 => 'تقليل الري لتفادي التعفن.';

  @override
  String get resources => 'مصادر موثوقة';

  @override
  String get youtubeChannels =>
      '• قنوات زراعة على يوتيوب: زراعة بلكونة، مهندس زراعي';

  @override
  String get needHelp => 'هل تحتاج مساعدة؟';

  @override
  String get contactExpertsInfo =>
      '• استخدم ميزة \'التواصل مع الخبراء\' داخل التطبيق.';

  @override
  String get powderyMildew => 'البياض الدقيقي';

  @override
  String get powderyMildewSymptoms => 'طبقة بيضاء على الأوراق';

  @override
  String get powderyMildewTreatment => 'تهوية جيدة + رش بالكبريت';

  @override
  String get confidence => 'الدقة';

  @override
  String get pick_camera => 'التقط صورة';

  @override
  String get lateBlight => 'اللفحة المتأخرة';

  @override
  String get lateBlightSymptoms => 'بقع سوداء على الطماطم';

  @override
  String get lateBlightTreatment => 'مبيد نحاسي + إزالة المصاب';

  @override
  String get rootRot => 'التعفن الجذري';

  @override
  String get rootRotSymptoms => 'اصفرار وموت تدريجي';

  @override
  String get rootRotTreatment => 'تحسين التصريف + تقليل الري';

  @override
  String get aphids => 'المن';

  @override
  String get aphidsSymptoms => 'حشرات صغيرة تمتص العصارة';

  @override
  String get aphidsTreatment => 'بخاخ النيم + ماء وصابون';

  @override
  String get takephoto => 'الكاميرا';

  @override
  String get selectimages => 'اختيار صورة';

  @override
  String get algal_leaf_spot_jackfruit => 'تبقع الأوراق الطحلبي في الجاك فروت';

  @override
  String get algal_leaf_spot_jackfruit_treatment =>
      'إزالة الأوراق المصابة وتحسين التهوية.\n استخدم اكسبرت تايم - المضفر - ديكتوب - الضجاج - سبيد - المعجزة\n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get anthracnose_mango => 'الأنثراكنوز في المانجو';

  @override
  String get anthracnose_mango_treatment =>
      'الرش بمبيد فطري نحاسي وتقليم الأجزاء المصابة.\n استخدم المتمكن - الحارس- ديكتوب - الضجاج - المعجزة\n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get aphids_cotton => 'حشرات المن في القطن';

  @override
  String get aphids_cotton_treatment =>
      'استخدام زيت النيم أو مبيد حشري مناسب.\n استخدم ناسف بلاس - الشرارة - كروش.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get apple_scab_apple => 'جرب التفاح';

  @override
  String get apple_scab_apple_treatment =>
      'الرش الوقائي بمبيدات فطرية.\n استخدم المتمكن - رايدستوم- الضجاج\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_blight_cotton => 'اللفحة البكتيرية في القطن';

  @override
  String get bacterial_blight_cotton_treatment =>
      'استخدام أصناف مقاومة وتجنب الري بالرش.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم - ديكتوب.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_canker_mango => 'التقرح البكتيري في المانجو';

  @override
  String get bacterial_canker_mango_treatment =>
      'تقليم الأفرع المصابة والرش بالنحاس.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_leaf_spot_pumpkin => 'التبقع البكتيري في القرع';

  @override
  String get bacterial_leaf_spot_pumpkin_treatment =>
      'إزالة النباتات المصابة وتدوير المحاصيل.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_spot_peach => 'التبقع البكتيري في الخوخ';

  @override
  String get bacterial_spot_peach_treatment =>
      'الرش بمبيدات بكتيرية وإزالة الأوراق المصابة.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم .\n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_spot_pepper_bell => 'التبقع البكتيري في الفلفل';

  @override
  String get bacterial_spot_pepper_bell_treatment =>
      'استخدام بذور سليمة والرش بالنحاس.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم- المعجزة.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get bacterial_spot_tomato => 'التبقع البكتيري في الطماطم';

  @override
  String get bacterial_spot_tomato_treatment =>
      'تجنب بلل الأوراق واستخدام مبيد نحاسي.\n استخدم اوكسي كلوريد النحاس - موس نتشر كوبر- اكسبرت تايم- المعجزة- المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get black_rot_cauliflower => 'العفن الأسود في القرنبيط';

  @override
  String get black_rot_cauliflower_treatment =>
      'إزالة النباتات المصابة وتحسين الصرف.\n اكسبرت تايم- المعجزة- المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get black_spot_jackfruit => 'البقعة السوداء في الجاك فروت';

  @override
  String get black_spot_jackfruit_treatment =>
      'إزالة الأوراق المصابة والرش الفطري.\n اكسبرت تايم- المعجزة- الحارس - الضجاج\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get black_rot_apple => 'العفن الأسود في التفاح';

  @override
  String get black_rot_apple_treatment =>
      'تقليم الأجزاء المصابة والرش الفطري.\n اكسبرت تايم- ديكتوب - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get black_rot_grape => 'العفن الأسود في العنب';

  @override
  String get black_rot_grape_treatment =>
      'إزالة العناقيد المصابة والرش.\n راديستوم - ديكتوب - الضجاج - المتمكن - المعجزة\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get brown_spot_rice => 'البقعة البنية في الأرز';

  @override
  String get brown_spot_rice_treatment =>
      'التسميد المتوازن والرش الفطري.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get cedar_apple_rust_apple => 'صدأ التفاح';

  @override
  String get cedar_apple_rust_apple_treatment =>
      'إزالة العوائل القريبة والرش الفطري.\n ديكتوب - الحارس - سبيد - المتمكن - المعجزة \nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get cercospora_leaf_spot_gray_leaf_spot_corn_maize =>
      'تبقع السركسبورا / البقعة الرمادية في الذرة';

  @override
  String get cercospora_leaf_spot_gray_leaf_spot_corn_maize_treatment =>
      'زراعة أصناف مقاومة والرش الفطري.\n فوس نتشر كوبر- اكسبرت تايم - ديكتوب - الضجاج - المعجزة\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get common_rust_corn_maize => 'الصدأ الشائع في الذرة';

  @override
  String get common_rust_corn_maize_treatment =>
      'زراعة هجن مقاومة.\n رايدستوم - اكسبرت تايم - ديكتوب - الضجاج - المعجزة - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get cutting_weevil_mango => 'سوسة القطع في المانجو';

  @override
  String get cutting_weevil_mango_treatment =>
      'إزالة الأفرع المصابة واستخدام مبيد حشري.\nالسفاح - الشرارة - زوكسي - الطاغية\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get die_back_mango => 'موت الأفرع في المانجو';

  @override
  String get die_back_mango_treatment =>
      'تقليم الأجزاء المصابة وتحسين التغذية.\nاكسبرت تيم - المعجزة - الحارس - فوس نتشر كوبر \n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get downy_mildew_pumpkin => 'البياض الزغبي في القرع';

  @override
  String get downy_mildew_pumpkin_treatment =>
      'تحسين التهوية والرش الفطري.\n سبيد - ديكتوب - اكسبرت تيم - ماكسفون - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get early_blight_potato => 'اللفحة المبكرة في البطاطس';

  @override
  String get early_blight_potato_treatment =>
      'إزالة الأوراق المصابة والرش.\n المتمكن - الضجاج - سبيد\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get early_blight_tomato => 'اللفحة المبكرة في الطماطم';

  @override
  String get early_blight_tomato_treatment =>
      'استخدام المهاد والرش الوقائي.\n المتمكن - الضجاج - سبيد\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get esca_black_measles_grape => 'الإسكا (الحصبة السوداء) في العنب';

  @override
  String get esca_black_measles_grape_treatment =>
      'إزالة الكروم المصابة وحماية أماكن التقليم.\n كارسبيد - المعجزة - اكسبرت تيم - ماكسفون - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get gall_midge_mango => 'ذبابة المرارة في المانجو';

  @override
  String get gall_midge_mango_treatment =>
      'استخدام مبيد حشري أثناء الإزهار.\n السفاح - كروش - الشرارة - المبتكر - اعدام\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get haunglongbing_citrus_greening_orange =>
      'اخضرار الحمضيات في البرتقال';

  @override
  String get haunglongbing_citrus_greening_orange_treatment =>
      'اولاً: مكافحة الحشرات الناقلة باحد المبيدات التالية\n الشرارة - السفاح - اعدام - الطاغية \nثانياً: مكافحة البكتريا المسببة باحد المبيدات التالية\n اوكسي كلوريد النحاس - اكسبرت تيم - الضجاج\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_cauliflower => 'نبات قرنبيط سليم';

  @override
  String get healthy_cauliflower_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_cotton => 'نبات قطن سليم';

  @override
  String get healthy_cotton_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_jackfruit => 'نبات جاك فروت سليم';

  @override
  String get healthy_jackfruit_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_mango => 'نبات مانجو سليم';

  @override
  String get healthy_mango_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_rice => 'نبات أرز سليم';

  @override
  String get healthy_rice_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_sugarcane => 'نبات قصب السكر سليم';

  @override
  String get healthy_sugarcane_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_leaf_pumpkin => 'ورقة قرع سليمة';

  @override
  String get healthy_leaf_pumpkin_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get hispa_rice => 'حشرة الهيسبا في الأرز';

  @override
  String get hispa_rice_treatment =>
      'استخدام مبيد حشري موصى به.\nالشرارة - السفاح - اعدام\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get late_blight_potato => 'اللفحة المتأخرة في البطاطس';

  @override
  String get late_blight_potato_treatment =>
      'إزالة النباتات المصابة والرش.\n كارسبيد - ديكتوب - الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get late_blight_tomato => 'اللفحة المتأخرة في الطماطم';

  @override
  String get late_blight_tomato_treatment =>
      'تجنب الري بالرش والرش الوقائي.\n كارسبيد - ديكتوب - اكسبرت تيم - الضجاج \nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get leaf_mold_tomato => 'عفن أوراق الطماطم';

  @override
  String get leaf_mold_tomato_treatment =>
      'تقليل الرطوبة وتحسين التهوية.\n ديكتوب - المتمكن - الضجاج - الجامح\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get leaf_blight_isariopsis_leaf_spot_grape => 'لفحة أوراق العنب';

  @override
  String get leaf_blight_isariopsis_leaf_spot_grape_treatment =>
      'إزالة الأوراق المصابة والرش الفطري.\n راديستوم - اكسبرت تيم -الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get leaf_scorch_strawberry => 'احتراق أوراق الفراولة';

  @override
  String get leaf_scorch_strawberry_treatment =>
      'تحسين التسميد وإزالة الأوراق المصابة.\nاكسبرت تيم -الضجاج - ديكتوب\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get leaf_blast_rice => 'لفحة أوراق الأرز';

  @override
  String get leaf_blast_rice_treatment =>
      'زراعة أصناف مقاومة والرش.\n اكسبرت تيم -الضجاج - الجامح\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get mosaic_sugarcane => 'مرض الفسيفساء في قصب السكر';

  @override
  String get mosaic_sugarcane_treatment =>
      'استخدام شتلات سليمة.\n ازالة النباتات المصابه خارج الحقل \n مكافحة النواقل باحد المبيدات التالية \n الشرارة - السفاح - ناسف - النبيل \nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get mosaic_disease_pumpkin => 'مرض الفسيفساء في القرع';

  @override
  String get mosaic_disease_pumpkin_treatment =>
      'ازالة النباتات المصابه خارج الحقل \n مكافحة النواقل باحد المبيدات التالية \n الشرارة - السفاح - اعدام\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get northern_leaf_blight_corn_maize =>
      'لفحة الأوراق الشمالية في الذرة';

  @override
  String get northern_leaf_blight_corn_maize_treatment =>
      'زراعة هجن مقاومة والرش.\n اكسبرت تيم - ديكتوب\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get powdery_mildew_cotton => 'البياض الدقيقي في القطن';

  @override
  String get powdery_mildew_cotton_treatment =>
      'استخدام مبيد فطري كبريتي.\n كاربتار - الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get powdery_mildew_mango => 'البياض الدقيقي في المانجو';

  @override
  String get powdery_mildew_mango_treatment =>
      'الرش أثناء الإزهار.\n كاربتار - الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get powdery_mildew_pumpkin => 'البياض الدقيقي في القرع';

  @override
  String get powdery_mildew_pumpkin_treatment =>
      'تحسين التهوية والرش.\n كاربتار - الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get powdery_mildew_qaad => 'البياض الدقيقي';

  @override
  String get powdery_mildew_qaad_treatment =>
      'استخدام مبيد فطري مناسب.\nكاربتار - اكسبرت تايم - ديكتوب - الضجاج - المضفر - الجامح\n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get powdery_mildew_cherry_sour => 'البياض الدقيقي في الكرز';

  @override
  String get powdery_mildew_cherry_sour_treatment =>
      'تقليم الأجزاء المصابة والرش.\n كاربتار - الضجاج - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get red_rot_sugarcane => 'العفن الأحمر في قصب السكر';

  @override
  String get red_rot_sugarcane_treatment =>
      'استخدام أصناف مقاومة.\n اكسبرت تيم - الحارس \nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get rust_sugarcane => 'الصدأ في قصب السكر';

  @override
  String get rust_sugarcane_treatment =>
      'الرش الفطري وتحسين النظافة.\n اكسبرت تيم - الحارس - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get septoria_leaf_spot_tomato => 'تبقع أوراق السيبتوريا في الطماطم';

  @override
  String get septoria_leaf_spot_tomato_treatment =>
      'إزالة الأوراق المصابة والرش.\n اكسبرت تيم - ديكتوب - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get sooty_mould_mango => 'العفن الأسود الهبابي في المانجو';

  @override
  String get sooty_mould_mango_treatment =>
      'اولاً: التخلص من الحشرات المسببه للندوة العسلية باستخدام.\n الشرارة - السفاح - اعدام - الحارس - المعجزة - ديكتوب \n ثانياً: التخلص من العفن المسبب للطبقة السوداء باستخدام اكسبرت تايم\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get spider_mites_two_spotted_spider_mite_tomato =>
      'العنكبوت الأحمر في الطماطم';

  @override
  String get spider_mites_two_spotted_spider_mite_tomato_treatment =>
      'استخدام احد المبيدات التالية .\n الطاغية - الداهية - الانفجار - كروش - الجاسوس - الشرارة \n ملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get target_spot_tomato => 'تبقع الهدف في الطماطم';

  @override
  String get target_spot_tomato_treatment =>
      'إزالة الأوراق المصابة والرش.\n الضجاج - اكسبرت تيم - المتمكن\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get target_spot_cotton => 'تبقع الهدف في القطن';

  @override
  String get target_spot_cotton_treatment =>
      'تدوير المحاصيل والرش.\n الضجاج - اكسبرت تيم - الحارس\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get tomato_yellow_leaf_curl_virus_tomato =>
      'فيروس تجعد واصفرار أوراق الطماطم';

  @override
  String get tomato_yellow_leaf_curl_virus_tomato_treatment =>
      'اولاً:تعقيم الأدوات وإزالة النباتات المصابة.\n ثانياً: مكافحة الناقل الحشري باستخدام الشرارة - السفاح - اعدام\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get tomato_mosaic_virus_tomato => 'فيروس موزاييك الطماطم';

  @override
  String get tomato_mosaic_virus_tomato_treatment =>
      'اولاً:تعقيم الأدوات وإزالة النباتات المصابة.\n ثانياً: مكافحة الناقل الحشري باستخدام الشرارة - السفاح - اعدام \nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get yellow_sugarcane => 'اصفرار قصب السكر';

  @override
  String get yellow_sugarcane_treatment =>
      'تحسين التسميد والصرف.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_apple => 'نبات تفاح سليم';

  @override
  String get healthy_apple_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_blueberry => 'نبات توت أزرق سليم';

  @override
  String get healthy_blueberry_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_cherry_sour => 'نبات كرز سليم';

  @override
  String get healthy_cherry_sour_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_corn_maize => 'نبات ذرة سليم';

  @override
  String get healthy_corn_maize_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_grape => 'نبات عنب سليم';

  @override
  String get healthy_grape_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_peach => 'نبات خوخ سليم';

  @override
  String get healthy_peach_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_pepper_bell => 'نبات فلفل سليم';

  @override
  String get healthy_pepper_bell_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_potato => 'نبات بطاطس سليم';

  @override
  String get healthy_potato_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_raspberry => 'نبات توت العليق سليم';

  @override
  String get healthy_raspberry_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_soybean => 'نبات فول صويا سليم';

  @override
  String get healthy_soybean_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_strawberry => 'نبات فراولة سليم';

  @override
  String get healthy_strawberry_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get healthy_tomato => 'نبات طماطم سليم';

  @override
  String get healthy_tomato_treatment =>
      'لا يحتاج إلى علاج.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get unknown_disease => 'مرض غير معروف / صورة غير نباتية';

  @override
  String get unknown_disease_treatment =>
      'يرجى تصوير ورقة نبات واضحة بإضاءة جيدة.\nملاحظة: للتأكد من دقة وسلامة التشخيص يمكنك الرجوع إلى صفحة الخبراء.';

  @override
  String get diagnosis_failed => 'فشل التشخيص.';

  @override
  String get pestsDiseases => 'الآفات والأمراض';

  @override
  String get pestsDiseases1 => 'الآفات والأمراض';

  @override
  String get selectCrop => 'اختر المحصول';

  @override
  String get noCropSelected => 'الرجاء اختيار محصول';

  @override
  String get stage => 'مرحلة';

  @override
  String get noDiseases => 'لا توجد أمراض';

  @override
  String get diseaseDetails => 'تفاصيل المرض';

  @override
  String get cause => 'الأسباب';

  @override
  String get preventiveMeasures => 'الإجراءات الوقائية';

  @override
  String get chemicalTreatment => 'المكافحة الكيميائية';

  @override
  String get alternativeTreatment => 'المكافحة العضوية';

  @override
  String get appTitle => 'تشخيص النبات';

  @override
  String get awareness => 'التوعية';

  @override
  String get aboutApp => 'حول التطبيق';

  @override
  String get selectCropFirst => 'الرجاء اختيار المحصول أولاً';

  @override
  String get noDiseasesFound => 'لا توجد أمراض لهذه المرحلة';

  @override
  String get selectStage => 'اختر المرحلة';

  @override
  String get causes => 'الأسباب';

  @override
  String get loading => 'جاري التحميل...';

  @override
  String get errorLoadingData => 'حدث خطأ أثناء تحميل البيانات.';

  @override
  String get tryAgain => 'إعادة المحاولة';

  @override
  String get noPreviousDiagnoses => 'لا توجد عناصر لعرضها حاليًا';

  @override
  String get previousDiagnos => 'التشخيصات السابقة';

  @override
  String get farmer_page_title => 'واجهة استشارات المزارع';

  @override
  String get tab_unanswered => 'استشارات لم يتم الرد عليها';

  @override
  String get tab_answered => 'استشارات تم الرد عليها';

  @override
  String get label_write_question => 'اكتب السؤال هنا';

  @override
  String get button_pick_image => 'اختر صورة';

  @override
  String get button_send_question => 'ارسال الاستفسار';

  @override
  String get snackbar_question_sent => 'تم ارسال الاستفسار ✅';

  @override
  String get label_question => 'السؤال:';

  @override
  String get label_answer => 'الإجابة:';

  @override
  String get filter_comment => 'تصفية بواسطة المزارع الحالي';

  @override
  String get previousQuestions => 'الأسئلة المجابة سابقًا';

  @override
  String get pendingQuestions => 'الأسئلة قيد الانتظار';

  @override
  String get button_record_audio => 'تسجيل صوتي';

  @override
  String get button_stop_recording => 'إيقاف التسجيل';

  @override
  String get label_audio_attached => 'تم إرفاق تسجيل صوتي';

  @override
  String get label_play_question_audio => 'سماع السؤال';

  @override
  String get label_play_answer_audio => 'سماع الرد';

  @override
  String get label_question_audio => 'الاستفسار';

  @override
  String get label_answer_audio => 'رد الخبير';

  @override
  String get label_delete_audio => 'حذف';

  @override
  String get label_no_image => 'لا توجد صورة';

  @override
  String get enterQuestion => 'ادخل السؤال';

  @override
  String get uploading => 'جاري الارسال';

  @override
  String get settings => 'الإعدادات';

  @override
  String get howToUse => 'طريقة استخدام التطبيق';

  @override
  String get privacyPolicy => 'سياسة الخصوصية';

  @override
  String get howToUseApp => 'آلية عمل التطبيق';

  @override
  String get help_intro =>
      'تتكون الصفحة الرئيسية في تطبيق المزارعين من خمسة أزرار:';

  @override
  String get help_diagnosis =>
      'يفتح واجهة التشخيص الآلي للنبات عن طريق صورة لورقة النبات.\n\nعند فتح الواجهة سيطلب منك التطبيق إذن الموقع لعرض المنطقة والطقس ودرجة الحرارة.\n\nلتشخيص المرض:\n- اضغط اختيار صورة\n- الكاميرا أو المعرض (الأفضل)\n\nيمكنك إرسال نفس الصورة للخبراء للتحقق.\n\nيظهر التشخيص مباشرة مع الدقة والعلاج.';

  @override
  String get help_experts =>
      'يمكن للمزارع إرسال استفسار نصي أو صورة أو صوت.\n\n- كتابة الرسالة في الحقل\n- إضافة صورة\n- تسجيل صوت\n\nيمكن مراجعة أو حذف الصوت.\n\nبعد الإرسال:\n- يظهر في الاستفسارات غير المجابة\n- يصل إشعار عند الرد\n- يظهر الرد لاحقاً (نصي أو صوتي)';

  @override
  String get help_pests =>
      'واجهة تحتوي على معلومات تفصيلية:\n\nالأعراض - الأسباب - المكافحة العضوية - المكافحة الكيميائية - الوقاية\n\nاختر محصول → مرحلة → مرض.';

  @override
  String get help_awareness => 'يعرض معلومات توعوية مختصرة للمزارع.';

  @override
  String get help_language =>
      'يدعم العربية والإنجليزية ويتم تحديث التطبيق فوراً.';

  @override
  String get help_settings =>
      'زر الإعدادات لعرض سياسة الخصوصية وآلية عمل التطبيق.';
}
